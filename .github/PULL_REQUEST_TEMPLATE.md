...
- [ ] `bash scripts/verify.sh` passes locally (or `uv run pytest && uv run ruff check . && uv run ruff format --check . && uv run mypy sov_engine sov_transport sov_cli sov_daemon`)
...
- [ ] Stayed within one domain (engine / transport / cli / tests / ci-tooling / frontend) — see CONTRIBUTING for the convention
- [ ] No secrets or wallet seeds in commits or test fixtures
- [ ] If adding/changing the proof envelope, bumped `PROOF_VERSION` and added a migration test (see `tests/test_proofs.py` patterns)
